...
NavierStokes::assemble_system(bool first_step)
{
    ...
    std::vector<Tensor<1, dim>> velocity_old(n_q);
    ...
    for (const auto &cell : dof_handler.active_cell_iterators())
      {
        ...
        fe_values[velocity].get_function_values(solution, velocity_current);
        fe_values[velocity].get_function_values(solution_old, velocity_old);
        fe_values[velocity].get_function_gradients(solution, velocity_current_gradients);
        fe_values[pressure].get_function_values(solution, pressure_current);
            ...
            for (unsigned int i = 0; i < dofs_per_cell; ++i)
              {
                ...
                cell_rhs(i) -= (velocity_current[q] - velocity_old[q]) /
                                  deltat * fe_values[velocity].value(i, q) *
                                  fe_values.JxW(q);
                ...
                cell_rhs(i) -= velocity_current_gradients[q] 
                            * velocity_current[q] 
                            * fe_values[velocity].value(i,q)
                            * fe_values.JxW(q);
...
NavierStokes::solve()
{
  ...
  // Apply the initial condition.
  {
    ...
    Functions::ZeroFunction<dim> u0(dim + 1);
